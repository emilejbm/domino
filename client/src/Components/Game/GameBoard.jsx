...
export default function GameBoard({ gameBoard, dominoCoords, startingMove, setDominoCoords }) {
    ...
    useMemo(() => {
        if (!gameBoard || !startingMove.domino) {
            setDominoCoords({});
            return;
        }
        ...
        const firstDomoIndex = gameBoard.findIndex(
            (domo) => domo.LeftSide === startingMove.domino.LeftSide && domo.RightSide === startingMove.domino.RightSide
        );
        let leftCounter = 0;
        let rightCounter = 0;
        ...
        for (let i = 0; i < gameBoard.length; i++) {
            ...
            } else if (gameBoard[firstDomoIndex - leftCounter - 1] !== undefined) {
                leftCounter++;
                domoToRenderIdx = firstDomoIndex - leftCounter;
            } else if (gameBoard[firstDomoIndex + rightCounter + 1] !== undefined) {
                rightCounter++;
                domoToRenderIdx = firstDomoIndex + rightCounter;
            }
    
            const { x, y } = getNewDomoPosition(true, domoToRenderIdx, firstDomoIndex, newCoords);
    ...
    }, [gameBoard, startingMove, setDominoCoords]);
    ...
    const dominoesJsx = useMemo(() => {
        const checkIfNewDomino = (domino, index) => {
            ...
            if (lastHead && domino.LeftSide == lastHead.LeftSide && domino.RightSide == lastHead.RightSide) {
                return false;
            }
            if (lastTail && domino.LeftSide == lastTail.LeftSide && domino.RightSide == lastTail.RightSide) {
                return false;
            }
        ...
        return gameBoard.map((domino, index) => {
            if (dominoCoords[`domino:${index}`] == undefined) {
                return
            }
...
const calcPositionForNewLeftmostDomino = (currIdx, coords) => {
    ...
    //     x = lastDominoOnLeftLocation.left - (DOMINO_WIDTH*2);
...
const getNewDomoPosition = (isDouble, currIdx, midIndex, coords) => {

if (currIdx < midIndex) { // move left of middle domo
    return calcPositionForNewLeftmostDomino(currIdx, coords);
} else if (currIdx > midIndex){ // move right of middle domo
    return calcPositionForNewRightmostDomino(currIdx, coords);
}
